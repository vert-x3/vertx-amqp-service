...
vertx.deployVerticle("service:io.vertx.vertx-amqp-service", options, function (res, res_err) {
  if (res_err == null) {
    // Deployed ok
  } else {
    // Failed to deploy
...
[source, js]
----
var AMQPService = require("vertx-amqp-js/amqp_service");
var service = AMQPService.createEventBusProxy(vertx, "vertx.service-amqp");
...
----
var AMQPService = require("vertx-amqp-js/amqp_service");
var outgoingLinkRef = null;
var service = AMQPService.createEventBusProxy(vertx, "vertx.service-amqp");
var options = {
};
options.reliability = 'AT_LEAST_ONCE';
service.establishOutgoingLink("amqp://localhost:6672/my-queue", "my-pub-queue", "my-pub-notifications", options, function (result, result_err) {
  if (result_err == null) {
    outgoingLinkRef = result;
...
service.cancelOutgoingLink(outgoingLinkRef, function (result, result_err) {
});
...
----
var AMQPService = require("vertx-amqp-js/amqp_service");
var incomingLinkRef = null;
var service = AMQPService.createEventBusProxy(vertx, "vertx.service-amqp");
var options = {
};
options.reliability = 'AT_LEAST_ONCE';
options.prefetch = 10;
service.establishIncomingLink("amqp://localhost:6672/my-queue", "my-sub-queue", "my-sub-notifications", options, function (result, result_err) {
  if (result_err == null) {
    incomingLinkRef = result;
  } else {
    //handle error
  }
});
...
service.cancelIncomingLink(incomingLinkRef, function (result, result_err) {
});
...
[source, js]
----
vertx.eventBus().consumer("my-pub-notifications", function (msg) {
  var type = Java.type("io.vertx.ext.amqp.NotificationHelper").getType(msg.body());
  if (type === 'LINK_CREDIT') {
    var msgsWeCanSend = Java.type("io.vertx.ext.amqp.NotificationHelper").getCredits(msg.body());
  }
});
...
var service;
var incomingLinkRef = null;
service.fetch(incomingLinkRef, 10, function (result, result_err) {
  if (result_err == null) {
...
service.registerService("fortune-cookie-service", "notice-address", options, function (result, result_err) {
  if (result_err == null) {
...
service.unregisterService("fortune-cookie-service", function (result, result_err) {
  if (result_err != null) {
    // error
  }
});
...
----
var service;
vertx.eventBus().consumer("notice-address", function (msg) {
  var type = Java.type("io.vertx.ext.amqp.NotificationHelper").getType(msg.body());
  if (type === 'INCOMING_LINK_OPENED') {
    var linkRef = Java.type("io.vertx.ext.amqp.NotificationHelper").getLinkRef(msg.body());
    service.issueCredits(linkRef, 1, function (result, result_err) {
    });
  }
});
